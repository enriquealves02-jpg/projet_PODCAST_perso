...
def format_articles_for_llm(articles: list[dict], offset: int = 0) -> str:
    ...
    for i, article in enumerate(articles):
        idx = offset + i
        content_preview = article["content"][:500] if article["content"] else "Pas de contenu disponible"
        lines.append(
            f"--- Article {idx} ---\n"
...
def filter_articles(articles: list[dict]) -> list[dict]:
    ...
    selected = [a for a in scored_articles if a["score"] >= MIN_SCORE][:MAX_SELECTED]
